...
import com.wsy.model.OrderAndGoodsInfo;

public class newGoodsCheckIFrame extends JInternalFrame {
	...
	private String[] columnNames={ "货物编号", "订货日期", "订货数量","操作员","是否验收","折扣","货物类别","货物名称","生产商","销售商","生产厂家","生产日期","货物价格"};
	private Map map=MapPz.getMap();
	private Object[][] getFileStates(List list){
		Object[][]results=new Object[list.size()][columnNames.length];
		for(int i=0;i<list.size();i++){
			OrderAndGoodsInfo order=(OrderAndGoodsInfo)list.get(i);
	...
	public newGoodsCheckIFrame() {
		...
		Object[][] results=getFileStates(Dao.selectGoodsOrder());
	...
	class CheckActionListener implements ActionListener{
		...
		public void actionPerformed(final ActionEvent e) {
			if(radioButton2.isSelected()){
				...
				int i=Dao.UpdateCheckGoodsOrder(ISBNs);
				if(i==1){
				...
				JOptionPane.showMessageDialog(null, "您选择的货物已经进行过验收，请选择其他货物进行验收");
			}
