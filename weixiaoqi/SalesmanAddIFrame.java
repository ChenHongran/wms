...
public class SalesmanAddIFrame extends JInternalFrame {
	...
	public SalesmanAddIFrame() {
		...
		major.setDocument(new MyDocument(30));
		panel_1.add(major);
	...
	class ButtonAddListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			...
			if(keepmoney.getText().length()==0){
				JOptionPane.showMessageDialog(null, "办证经费文本框不可为空");
				return;
			}
			if(major.getText().length()==0){
				JOptionPane.showMessageDialog(null, "毕业专业文本框不可为空");
				return;
			}
			if(major.getText().length()>20){
				JOptionPane.showMessageDialog(null, "毕业专业文本框位数为20");
				return;
			}
			...
			int i=Dao.InsertSalesman(readername.getText().trim(), sex.trim(), age.getText().trim(),zjnumber.getText().trim(), Date.valueOf(date.getText().trim()), maxnumber.getText().trim(),tel.getText().trim(), Double.valueOf(keepmoney.getText().trim()),zj,major.getText().trim(),Date.valueOf(bztime.getText().trim()),ISBN.getText().trim());
