...
import com.wsy.model.Salesman;
import com.wsy.util.CreatecdIcon;
import com.wsy.util.MyDocument;

public class SalesmanModiAndDelIFrame extends JInternalFrame {
	...
	private String[] columnNames={ "销售人员名称", "销售人员性别", "销售人员年龄", "证件号码", "证件有效日期",
			"最大调货量", "电话", "工资", "证件", "专业", "销售人员编号" ,"办证日期"};
	private String[] array=new String[]{"身份证","工作证", " ", " "};
	String id;
	...
	class ModiButtonListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			...
			int i=Dao.UpdateSalesman(id, readername.getText().trim(), sex, age.getText().trim(), zjnumber.getText().trim(), Date.valueOf(date.getText().trim()), maxnumber.getText().trim(), tel.getText().trim(), Double.valueOf(keepmoney.getText().trim()), zj, zy.getText().trim(), Date.valueOf(bztime.getText().trim()), ISBN.getText().trim());
