...
import com.wsy.model.GoodsInfo;
import com.wsy.model.Operater;
import com.wsy.util.MyDocument;

public class newGoodsOrderIFrame extends JInternalFrame {
	...
	private Operater user = WMSLoginIFrame.getUser(); 


	/**
	 * Create the frame
	 */
	public newGoodsOrderIFrame() {
		super();
		setTitle("新货订购管理");
		setIconifiable(true);
		setClosable(true);
		...
		panel_4.setBorder(new TitledBorder(null, "订货信息", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, null, null));
		...
		panel_1.setBorder(new TitledBorder(null, "货物信息", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, null, null));
		...
		String[] array=new String[]{"***生产厂家","**有限公司","**集团","***小型公司"};
	...
	class ButtonAddLisenter implements ActionListener{
		public void actionPerformed(final ActionEvent e) {
			if(orderDate.getText().isEmpty()){
				JOptionPane.showMessageDialog(null, "订货日期文本框不可为空");
				return;
			}
			if(ISBN.getText().isEmpty()){
				JOptionPane.showMessageDialog(null, "货物编号文本框不可为空");
				return;
			}
			if(orderNumber.getText().isEmpty()){
				JOptionPane.showMessageDialog(null, "订货数量文本框不可为空");
				return;
			}
			...
			if(!Dao.selectGoodsOrder(ISBN.getText().trim()).isEmpty()){
				JOptionPane.showMessageDialog(null, "添加货物号重复！");
				return;
			}
			...
			try{
				int i=Dao.InsertGoodsOrder(ISBN.getText().trim(), java.sql.Date.valueOf(orderDate.getText().trim()), orderNumber.getText().trim(), operator.getText().trim(), checkAndAccept,zks);
	...
	class ISBNListener extends KeyAdapter {//使用回车键进行触发事件方法
		public void keyTyped(KeyEvent e) {
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				...
				List list = Dao.selectGoodsInfo(ISBNs);
				System.out.println(list.isEmpty());
				if(list.isEmpty()){
					new GoodsAddIFrame();
				}
				for (int i = 0; i < list.size(); i++) {
					GoodsInfo bookinfo = (GoodsInfo) list.get(i);
					bookName.setText(bookinfo.getGoodsname());
	...
	class ISBNListenerlostFocus extends FocusAdapter{
		public void focusLost(FocusEvent e){
			String ISBNs = ISBN.getText().trim();
			if(!Dao.selectGoodsOrder(ISBN.getText().trim()).isEmpty()){
				JOptionPane.showMessageDialog(null, "已经为此编号货物添加订货信息，请输入其他货物编号！");
				ISBN.setText("");
				bookName.setText("");
				price.setText("");
				return;
			...
			if(list.isEmpty()&&!ISBN.getText().isEmpty()){
				...
				bookName.setText("");
				price.setText("");
				JOptionPane.showMessageDialog(null, "货物信息表中无此书号，请您首先到基础数据维护中进行货物信息添加操作");
			}
			for (int i = 0; i < list.size(); i++) {
				GoodsInfo bookinfo = (GoodsInfo) list.get(i);
				bookName.setText(bookinfo.getGoodsname());
