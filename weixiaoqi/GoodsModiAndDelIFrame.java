 ...
 *
 */
public class GoodsModiAndDelIFrame extends JInternalFrame {
	...
	private Object[][] getFileStates(List list){
		String[] columnNames = { "货物编号", "货物类别", "货物名称", "生产商", "销售商", "生产厂家",
				"生产日期", "价格" };
		...
		for(int i=0;i<list.size();i++){
			...
			results[i][2]=bookinfo.getGoodsname();
...
//		panel_1.add(button_2);
		...
		Object[][] results=getFileStates(Dao.selectGoodsInfo());
		columnNames = new String[]{"货物编号", "货物类别", "货物名称", "生产商", "销售商", "生产厂家", "生产日期",
				"价格"};
	...
	class addBookActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			if(ISBN.getText().length()==0){
				JOptionPane.showMessageDialog(null, "货物编号文本框不可以为空或则输入数字不可以大于13个");
				return;
			}
			...
			if(bookName.getText().length()==0){
				JOptionPane.showMessageDialog(null, "货物名称文本框不可以为空");
				return;
			}
			...
			if(publisher.getText().length()==0){
				JOptionPane.showMessageDialog(null, "生产厂家文本框不可以为空");
				return;
			}
			//日期与单价进行数字验证代码？
			if(pubDate.getText().length()==0){
				JOptionPane.showMessageDialog(null, "生产日期文本框不可以为空");
				return;
			}
			...
			int i=Dao.Updategoods(ISBNs, bookTypes, bookNames, writers, translators, publishers, Date.valueOf(pubDates), Double.parseDouble(prices));
